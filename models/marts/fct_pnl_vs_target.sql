with
    pnl_by_desk as (select * from {{ ref("fct_pnl_by_desk") }}),

    weights as (select * from {{ ref("stg_weights") }}),
    ...
    total_portfolio_value as (
        select trade_date, sum(portfolio_value_usd) as total_portfolio_value_usd
    ...
    allocation_variance as (
        select
            ...
            pv.portfolio_value_usd
            / nullif(tp.total_portfolio_value_usd, 0) as actual_allocation,
            (
                pv.portfolio_value_usd / nullif(tp.total_portfolio_value_usd, 0)
                - w.target_allocation
        ...
        join total_portfolio_value tp on pv.trade_date = tp.trade_date
        join weights w on pv.desk = w.desk and pv.region = w.region
    )
