...
with
    extracted_entities as (select * from {{ ref("int_extracted_entities") }}),
    ...
    trader_totals as (
        select trader_name, sum(total_trades) as total_trades
    ...
    final_stats as (
        select
            ...
            tds.signals_used,
            round(
                tds.total_trades * 100.0 / nullif(tt.total_trades, 0), 2
            ) as driver_percentage
        from trader_driver_stats tds
        join trader_totals tt on tds.trader_name = tt.trader_name
    )
